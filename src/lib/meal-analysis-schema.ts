...
const rawMicronutrientsSchema = z.object({
  ...
  omega3_mg: z.number().min(0).max(10000),
...
export const micronutrientsSchema = z.object({
  ...
  magnesiumMg: z.number().min(0).max(2000),
  ironMg: z.number().min(0).max(100),
  zincMg: z.number().min(0).max(100),
  omega3Mg: z.number().min(0).max(10000),
...
function normalizeMicronutrients(raw: z.infer<typeof rawMicronutrientsSchema>) {
  return micronutrientsSchema.parse({
    ...
    magnesiumMg: roundToSingleDecimal(raw.magnesium_mg),
    ironMg: roundToSingleDecimal(raw.iron_mg),
    zincMg: roundToSingleDecimal(raw.zinc_mg),
    omega3Mg: roundToSingleDecimal(raw.omega3_mg),
    ...
    micronutrients: {
      ...
      properties: {
        ...
        magnesium_mg: {
          ...
          minimum: 0,
          maximum: 2000,
          description: "Estimated magnesium in milligrams.",
        },
        iron_mg: {
          type: "number",
          minimum: 0,
        ...
        omega3_mg: {
          ...
          minimum: 0,
          maximum: 10000,
          description: "Estimated omega-3 fatty acids in milligrams.",
        },
        vitamin_c_mg: {
          type: "number",
          minimum: 0,
